    ...
    try {
        ...
        if (itemIndex > -1) {
            ...
            cart.items[itemIndex].price =
                cart.items[itemIndex].qty * cart.items[itemIndex].onePrice;
            cart.totalQty++;
            // ! total cost
            cart.totalCost += cart.items[itemIndex].onePrice;
        } else {
            ...
            if (type) {
                ...
                if (product.price > 1000 && product.price <= 5000) {
                    varTax = product.price * 0.12;
                } else if (product.price > 5000) {
                    varTax = product.price * 0.18;
                }
            } else {
                fixTax = 100;
                if (product.price > 1000 && product.price <= 8000) {
                    varTax = product.price * 0.1;
                } else if (product.price > 8000) {
                    varTax = product.price * 0.15;
                }
            }
            let totalPrice = fixTax + varTax + product.price;
...
// Delete Cart
router.get('/deleteCart', async function (req, res, next) {
    let cart;
    try {
        if (req.user) {
            cart = await Cart.findOne({ user: req.user._id });
...
async function productsFromCart(cart) {
    ...
    for (const item of cart.items) {
        ...
        foundProduct['varTax'] = item.varTax;
        foundProduct['fixTax'] = item.fixTax;
